{{-- <div id="footer">
    <div class="footer_left">
        <fieldset class="info_fieldset">
            <legend class="info-heading">MẠNG XÃ HỘI</legend>
            <svg xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" fill="currentColor"
                class="bi bi-facebook" viewBox="0 0 16 16">
                <path
                    d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951" />
            </svg>
            <svg xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" fill="currentColor"
                class="bi bi-instagram" viewBox="0 0 16 16">
                <path
                    d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599.28.28.453.546.598.92.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598 2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233 0-2.136.008-2.388.046-3.231.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92.28-.28.546-.453.92-.598.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334" />
            </svg>
            <svg xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" fill="currentColor"
                class="bi bi-messenger" viewBox="0 0 16 16">
                <path
                    d="M0 7.76C0 3.301 3.493 0 8 0s8 3.301 8 7.76-3.493 7.76-8 7.76c-.81 0-1.586-.107-2.316-.307a.639.639 0 0 0-.427.03l-1.588.702a.64.64 0 0 1-.898-.566l-.044-1.423a.639.639 0 0 0-.215-.456C.956 12.108 0 10.092 0 7.76m5.546-1.459-2.35 3.728c-.225.358.214.761.551.506l2.525-1.916a.48.48 0 0 1 .578-.002l1.869 1.402a1.2 1.2 0 0 0 1.735-.32l2.35-3.728c.226-.358-.214-.761-.551-.506L9.728 7.381a.48.48 0 0 1-.578.002L7.281 5.98a1.2 1.2 0 0 0-1.735.32z" />
            </svg>
            <!-- <a href="#"><i class="bi bi-facebook"></i></a>
            <a href="#"><i class="bi bi-instagram"></i></a>
            <a href="#"><i class="bi bi-messenger"></i></a> -->
        </fieldset>

        <div class="info_company">
            <h3 class="info_company_name">CÔNG TY TNHH TIENPC</h3>
            <p class="info_company_desc">Số GCNĐKDN: 123456789 <br>
                Cấp lần đầu: Ngày 21/03/2020 <br>
                Đăng ký thay đổi lần thứ 19: Ngày 11/11/2023 <br>
                Cơ quan cấp: Sở kế hoạch và đầu tư tỉnh Quảng Ninh <br>
                Địa chỉ: Số 121 Đường Trần Hưng Đạo, TP Hạ Long, Quảng Ninh
            </p>

        </div>
    </div>

    <div class="footer_right">
        <div class="info-phone">
            <h1 class="info-phone-heading">Đường dây nóng</h1>
            <div class="info-phone-list">
                <div class="info-phone-icon">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                        class="bi bi-telephone" viewBox="0 0 16 16">
                        <path
                            d="M3.654 1.328a.678.678 0 0 0-1.015-.063L1.605 2.3c-.483.484-.661 1.169-.45 1.77a17.568 17.568 0 0 0 4.168 6.608 17.569 17.569 0 0 0 6.608 4.168c.601.211 1.286.033 1.77-.45l1.034-1.034a.678.678 0 0 0-.063-1.015l-2.307-1.794a.678.678 0 0 0-.58-.122l-2.19.547a1.745 1.745 0 0 1-1.657-.459L5.482 8.062a1.745 1.745 0 0 1-.46-1.657l.548-2.19a.678.678 0 0 0-.122-.58L3.654 1.328zM1.884.511a1.745 1.745 0 0 1 2.612.163L6.29 2.98c.329.423.445.974.315 1.494l-.547 2.19a.678.678 0 0 0 .178.643l2.457 2.457a.678.678 0 0 0 .644.178l2.189-.547a1.745 1.745 0 0 1 1.494.315l2.306 1.794c.829.645.905 1.87.163 2.611l-1.034 1.034c-.74.74-1.846 1.065-2.877.702a18.634 18.634 0 0 1-7.01-4.42 18.634 18.634 0 0 1-4.42-7.009c-.362-1.03-.037-2.137.703-2.877L1.885.511z" />
                    </svg>
                    <span>09 35 35 67 89</span>
                </div>

                <div info-phone-icon>
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                        class="bi bi-envelope" viewBox="0 0 16 16">
                        <path
                            d="M0 4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2zm2-1a1 1 0 0 0-1 1v.217l7 4.2 7-4.2V4a1 1 0 0 0-1-1zm13 2.383-4.708 2.825L15 11.105zm-.034 6.876-5.64-3.471L8 9.583l-1.326-.795-5.64 3.47A1 1 0 0 0 2 13h12a1 1 0 0 0 .966-.741M1 11.105l4.708-2.897L1 5.383z" />
                    </svg>
                    <span>contact.tienpc@gmail.com</span>
                </div>
            </div>
        </div>

        <div class="policy">
            <h1 class="policy-main-heading">Chính sách</h1>
            <div class="policy_list">
                <h4 class="policy-heading"><a href="" class="policy-link">Chính sách Thanh toán</a></h4>
                <h4 class="policy-heading"><a href="" class="policy-link">Chính sách Bảo mật</a></h4>

            </div>
        </div>
    </div>

</div> --}}

<footer>
    <div class="top-footer">
        <div class="container ">
            <div class="row ">
                <div class="col-md-6">
                    <h3>Trải nghiệm mua sắm tuyệt vời</h3>
                    <p>Mang đến cho bạn những sản phẩm rẻ nhất và chất lượng nhất</p>
                </div>
                <div class="col-md-2"></div>
                <div class="col-md-4 ">
                    @if (Route::has('register'))
                <a href="{{ route('register') }}" class="btn btn primary">Đăng ký ngay</a>
                    @endif
                </div>
            </div>
        </div>


    </div>
    </div>
    <div class="bottom-footer ">
        <div class="container">
            <div class="row">
                <div class="col-md-4">
                    <h4>DỊCH VỤ KHÁCH HÀNG</h4>
                    <p>Từ Thứ 2 đến Thứ 7</p>
                    <p>10am - 6.30pm</p>
                    <p>033 7677 141</p>
                    <p>sostien0409@gmail.com</p>
                </div>
                <div class="col-md-4">
                    <h4>CHÍNH SÁCH</h4>
                    <p>Giao hàng nhanh trong vòng 3 ngày</p>
                    <p>Hoàn trả trong vòng 1 tháng</p>
                    <p>Thanh toán khi nhận hàng</p>

                </div>
                <div class="col-md-4">
                    <h4>CÔNG TY</h4>
                    <p>CÔNG TY TNHH TMDV TIENPC</p>
                    <p>SỐ 123, Đường X, Phường Y, TP Z</p>
                    <p>Liên hệ</p>
                    <p>Điều khoản và dịch vụ</p>
                </div>


            </div>

            <div class="row">

                <div class="col-md-4">
                    <p>© 2023. Design by Tiensv3</p>
                </div>
                {{-- <div class="col-md-4">
                    <img src="img/footer/footer1.png" alt="payment">
                </div> --}}
                <div class="col-md-4">
                    <a href=""><i class="ti-facebook"></i></a>
                    <a href=""><i class="ti-youtube"></i></a>
                    <a href=""><i class="ti-twitter"></i></a>
                    <a href=""><i class="ti-instagram"></i></a>
                </div>

            </div>
        </div>


    </div>

</footer>
@push('script')
<script src="{{ asset('user/js/jquery-3.3.1.slim.min.js') }}"></script>
<script src="{{ asset('user/js/bootstrap.min.js') }}"></script>
<script src="{{ asset('user/js/index.js') }}"></script>
{{-- <script src="{{ asset('user/js/slideshow.js') }}"></script> --}}
<script src="{{ asset('user/js/imageChange.js') }}"></script>
<script src="{{ asset('user/js/updateQuantity.js') }}"></script>
<script src="{{ asset('user/js/updateTotal.js') }}"></script>
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>

@endpush

